...
# reward state: retrieval

def reward_retrieval(event):
    if event == 'entry':
        hw.reward_port.LED.on()
        hw.syringe_pump.backward(v.steps_rate, v.n_steps)
...
# reward state: limited hold

def reward_hold(event):
    if event == 'entry':
        timed_goto_state('ITI', v.reward_stimulation_dur)
